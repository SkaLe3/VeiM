...
// This border is going to be drawn even if window border size is set to 0.0f
	...
	void Utils::SetCharArrayData(char* destinationLocation, size_t destinationSize, const String& sourceData)
	{
		strncpy(destinationLocation, sourceData.c_str(), destinationSize - 1);
		destinationLocation[destinationSize - 1] = '\0';
	}
	...
	String Utils::WStringToString(const std::wstring& wstr)
	{
		...
		WideCharToMultiByte(CP_UTF8, 0, wstr.c_str(), -1, &str[0], requiredSize, nullptr, nullptr);
		return str;
	}
