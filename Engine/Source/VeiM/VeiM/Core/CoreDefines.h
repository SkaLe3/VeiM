...
#define VM_BIND_EVENT(fn) [this](auto&&... args) -> decltype(auto) {return this->fn(std::forward<decltype(args)>(args)...);}
	...
	#define	VM_INTERNAL_ASSERT_NO_MSG(type, check) VM_INTERNAL_ASSERT_IMPL(type, check, "Assertion '{0}' failed at {1}:{2}", VM_STRINGIFY_MACRO(check), std::filesystem::path(__FILE__).filename().filename().wstring(), __LINE__)
