...
bool GenerateFile(const std::filesystem::path& filePath, const std::wstring& comment, const std::wstring& content)
{
	std::wstring fileName = filePath.stem().filename().wstring();
...
bool GenerateProjectFiles(const std::wstring& projectFileName)
{
	...
	std::wstring fileName = TEXT("EngineRoot.lua");
	std::filesystem::path filePath = std::filesystem::path(Paths::GetPath(projectFileName)) / fileName;
	std::wstring comment = std::wstring(TEXT("-- This file is dynamically generated by VeiM Manager Tool during project files generation with Premake\n-- "))
												+ fileName + TEXT(" contains the path to the associated engine installation directory.");
	std::wstring content = std::wstring(TEXT("engine_root_dir = \"")) + rootDir + TEXT("\"");

	if (!GenerateFile(filePath, comment, content))
	{
		std::wstring errorMessage = std::wstring(TEXT("Failed to generate ")) + fileName;
		MessageBoxW(NULL, errorMessage.c_str(), TEXT("Error"), MB_OK | MB_ICONERROR);
		return false;
	}
