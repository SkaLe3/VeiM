...
#include <fstream> // TODO: Delete this, and add YAML (YAML wrapper in VeiM)
...
// TODO: Change to use IFileDialog
bool DesktopPlatformWindows::OpenFileDialog(const void* parentWindowHandle, const std::wstring& dialogTitle, const std::wstring& defaultPath, const std::wstring& defaultFile, const std::wstring& fileTypes, std::wstring& outFilename)
{
	...
	std::vector<TCHAR> persistentFilter;
	...
	else
	{
		...
		std::wstringstream ss(fileTypes);
		std::wstring item;
		...
		if (values.size() % 2 == 0)
		{
			...
			{
				persistentFilter.insert(persistentFilter.end(), val.begin(), val.end());
				persistentFilter.push_back(TEXT('\0'));
			}
			ofn.lpstrFilter = persistentFilter.data();
		}
	}
	...
	{
		wcsncpy_s(szFile, defaultFile.c_str(), sizeof(szFile));
	}
...
bool DesktopPlatformWindows::OpenDirectoryDialog(const void* parentWindowHandle, const std::wstring& dialogTitle, const std::wstring& defaultPath, std::wstring& outDirectoryName)
{
	bool bSuccess = false;
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			// Set the dialog options to select folders instead of files
			DWORD dwOptions;
			...
			if (!defaultPath.empty())
			{
				...
				hr = SHCreateItemFromParsingName(defaultPath.c_str(), NULL, IID_PPV_ARGS(&pDefaultFolder));
				if (SUCCEEDED(hr))
				{
			...
			// Show the dialog
			hr = pFileDialog->Show((HWND)glfwGetWin32Window((GLFWwindow*)parentWindowHandle));
			if (SUCCEEDED(hr))
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					if (SUCCEEDED(hr))
					{
						...
						std::wstring directoryName = pszFilePath;
						outDirectoryName = directoryName;
